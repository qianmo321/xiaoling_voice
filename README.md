...
功能：唤醒词与待机（含语音播报）、场景切换（机器人展厅/银座/清水寺）、中日双语、
本地知识库（简介路由+长上下文，不切片）、Tavily 联网检索、Open-Meteo 天气查询（默认城市跟随场景）、
打断需唤醒词、语气词/幻觉过滤、断线自动重连。
...
├── core/                ← 共享核心：kb.py（知识库）、weather.py（天气）、scenes.json（场景定义）
...
服务器部署与维护（HTTPS 证书、systemd 常驻、更新流程、排错）见 **`DEPLOY.md`**；
功能说明与测试指南（给测试人员）见 **`FEATURES.md`**。

## 日常修改
| 改什么 | 位置 |
|--------|------|
| 场景人设/别名/默认天气城市/通用规则 | `core/scenes.json` → 重启 |
...
## 测试
```bash
cd desktop_test && python wake_logic_test.py     # 唤醒/过滤状态机（离线，36 用例）
cd web_voice   && python interrupt_test.py       # 打断门槛（离线，5 用例）
cd web_voice   && python ws_test.py              # 网页版全链路（需服务已启动）
cd web_voice   && python lang_test.py            # 中日语言切换（需服务已启动）
cd core        && python weather.py 東京 2 ja    # 天气模块单测（城市/天数/语言）
```
